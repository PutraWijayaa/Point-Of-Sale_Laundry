...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["update_name"])) {
    ...
    if (!empty($new_name)) {
        ...
        if ($stmt->execute()) {
          header("Location: ?page=profile&alert=success&message=Nama berhasil diperbarui!");
            // echo "<script>alert('Nama berhasil diperbarui!'); window.location.href='?page=profile';</script>";
        } else {
          header("Location: ?page=profile&alert=error&message=Gagal memperbarui nama.");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["update_password"])) {
    ...
    if (!password_verify($current_password, $user_data["password"])) {
        header("Location: ?page=profile&alert=error&message=Password lama salah!");
        // echo "<script>alert('Password lama salah!');</script>";
    } elseif ($new_password !== $confirm_password) {
      header("Location: ?page=profile&alert=error&message=Konfirmasi password tidak cocok!");
        ...
        if ($stmt->execute()) {
          header("Location: ?page=profile&alert=success&message=Password berhasil diperbarui!");
            // echo "<script>alert('Password berhasil diperbarui!'); window.location.href='?page=profile';</script>";
        } else {
          header("Location: ?page=profile&alert=error&message=Gagal memperbarui password.");
                    ...
                    <ul class="nav nav-tabs nav-tabs-bordered" role="tablist">
                        ...
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" data-bs-toggle="tab" data-bs-target="#profile-settings"
                                aria-selected="false" role="tab" tabindex="-1">Update Name</button>
                        </li>

                        <li class="nav-item" role="presentation">
                            <button class="nav-link active" data-bs-toggle="tab"
                                data-bs-target="#profile-change-password" aria-selected="true" role="tab">Change
                                Password</button>
                        </li>
